<template>
  <div class="q-pa-md">
    <q-btn label="Add" color="primary" @click="addRedfishHostModel = true" />
    <q-dialog v-model="addRedfishHostModel">
      <q-card style="min-width: 350px">
        ...
        <q-card-actions align="right" class="text-primary">
          ...
          <q-btn flat label="Add redfish host" type="submit"/>
        </q-card-actions>
        </q-form>
      </q-card>
    </q-dialog>
    <q-table
      title="Redfish agents"
      :rows="rows"
      :columns="columns"
      row-key="name"
    />
...
export default {
  ...
  methods: {
    ...mapActions('auth', ['addRedfishHost']),
   async onSubmit () {
     const ok = await this.addRedfishHost(this.host,this.username,this.password)
     if (ok) {
       this.q.notify({
         color: 'green-4',
         textColor: 'white',
    ...
    getAgents() {
        Api.get('/redfish/agents').then(resp =>
  ...
  setup () {
    ...
    const addRedfishHostModel = ref(null)
    return {
      columns,
      rows,
      addRedfishHostModel,
